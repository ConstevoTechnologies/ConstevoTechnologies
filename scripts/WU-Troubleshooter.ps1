...
function Get-LastInstalledUpdate {
    ...
    try {
        ...
        if ($lastHotfix) {
            # InstalledOn peut être $null sur certaines machines — compatibilité PS 5.1
            $installedOnStr = if ($lastHotfix.InstalledOn) {
            ...
            Write-Log ("  Installé le : {0}" -f $installedOnStr)         "DATA"
...
function Get-WUConfiguration {
    ...
    # Serveur WSUS configuré ? — compatible PS 5.1 (pas de ?. )
    $wsusReg    = Get-ItemProperty "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" `
                      -Name "WUServer" -ErrorAction SilentlyContinue
    $wsusServer = if ($wsusReg) { $wsusReg.WUServer } else { $null }
...
function Get-PendingReboot {
    ...
    # Pending file rename — compatible PS 5.1 (pas de ?. )
    $pfroReg = Get-ItemProperty `
        "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager" `
        -Name "PendingFileRenameOperations" -ErrorAction SilentlyContinue
    $pfro = if ($pfroReg) { $pfroReg.PendingFileRenameOperations } else { $null }
    if ($pfro) { $rebootSources += "Pending File Rename Operations" }

    # SCCM — compatible PS 5.1 (pas de ?. )
    try {
        $sccmReboot = ([wmiclass]"\\.\root\ccm\clientsdk:CCM_ClientUtilities").DetermineIfRebootPending()
        if ($sccmReboot -and $sccmReboot.RebootPending) { $rebootSources += "SCCM / ConfigMgr" }
    } catch {
        # SCCM non installé — on ignore silencieusement
    }
